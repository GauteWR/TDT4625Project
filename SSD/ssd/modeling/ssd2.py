...
class SSD300(nn.Module):
    def __init__(self, 
        ...
        for n_boxes, out_ch in zip(anchors.num_boxes_per_fmap, self.feature_extractor.out_channels):
            # Add deeper conv nests for regression head and classification head
    ...
    def _init_weights_improved(self):
        ...
        for layer in self.regression_heads:
            for l in layer:
                if not isinstance(l, nn.ReLU) and not isinstance(l, nn.Sequential):
                    l.bias.data.fill_(0)
        ...
        p = 0.995
        self.classification_heads[-1][0].bias.data[:len(self.classification_heads[-1][0].bias)//9], np.log(p*(self.num_classes - 1)/(1-p))
